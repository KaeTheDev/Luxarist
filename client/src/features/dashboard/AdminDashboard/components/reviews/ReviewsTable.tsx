/**
 * Purpose: Renders the admin testimonial moderation table.
 *
 * Responsibilities:
 * - Fetch all reviews from GET /api/admin/reviews on mount.
 * - Render a desktop table via ReviewRow and a mobile card layout.
 * - Handle optimistic approval toggling via PUT /api/admin/reviews/:id,
 *   rolling back local state if the server call fails.
 *
 * Usage:
 *   <ReviewsTable />  (mounted via AdminDashboard route at /admin/reviews)
 */
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-2">
        <div>
          ...
          <p className="text-sm text-stone-500 font-light mt-2 max-w-md">Managing the public voice of the Luxarist brand.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-275">
            <thead>
              <tr className="border-b border-stone-50 bg-stone-50/30 text-stone-400 font-black italic">
                ...
                <th className="px-10 py-8 text-[9px] uppercase tracking-[0.4em]">Title</th>
            ...
            <tbody className="divide-y divide-stone-50">
              {reviews.length > 0 ? (
                reviews.map((review) => (
                  <ReviewRow key={review._id} review={review} onToggle={handleToggleApproval} />
                ))
              ) : (
                <tr>
                  <td colSpan={7} className="px-10 py-32 text-center text-stone-300 font-serif italic text-lg">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h4 className="font-serif italic text-stone-900 text-lg">
                    {review.productId?.name ?? "Luxarist Piece"}
                  </h4>
                  {review.title && (
                    <p className="text-xs text-stone-600 font-medium mt-1">{review.title}</p>
                  )}
                </div>
