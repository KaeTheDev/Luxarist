...
export default function DashboardShell({ children, role, title }: DashboardShellProps) {
  ...
  return (
    <div className="flex h-screen bg-white text-stone-900 overflow-hidden font-sans">
              ...
              <Link
                ...
                className={`flex items-center justify-between px-5 py-4 rounded-2xl group ${
                  isActive
                    ...
                    : "text-stone-400 hover:text-stone-900 hover:bg-stone-50/50 transition-colors duration-200"
                }`}
              >
                <div className="flex items-center gap-4">
                  <span className={`${isActive ? "scale-100" : "group-hover:scale-110 transition-transform"}`}>
                    {link.icon}
                  </span>
                  <span className="text-[10px] uppercase tracking-[0.15em] font-bold">{link.label}</span>
                </div>
                {isActive && (
                  <ChevronRight size={14} className="text-stone-900 animate-in fade-in zoom-in duration-300" />
                )}
              </Link>
            );
          })}
      ...
      {isSidebarOpen && (
        <div
          className="fixed inset-0 bg-stone-900/5 backdrop-blur-sm z-40 lg:hidden"
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-24 flex items-center justify-between px-8 lg:px-16 bg-white/70 backdrop-blur-xl sticky top-0 z-10 border-b border-stone-50">
          <div className="flex items-center gap-4 lg:gap-8">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 -ml-2 text-stone-400 hover:text-stone-900 transition-colors">
              <Menu size={22} />
            </button>
            ...
            <h2 className="text-sm font-serif italic text-stone-400 uppercase tracking-tight truncate max-w-150 sm:max-w-none">
              {title}
            </h2>
          </div>
